...
ota_update_task(void * pvParameter)
{
    ...
    if (configured_part != running_part) {
        ...
        ESP_LOGW(TAG, "Configured OTA boot partition at offset 0x%08x, but running from offset 0x%08x",
                 (unsigned int)configured_part->address, (unsigned int)running_part->address);
    }
    ESP_LOGI(TAG, "Running from part \"%s\" (0x%08x)", running_part->label, (unsigned int)running_part->address);
    ...
    ESP_LOGI(TAG, "Writing to part \"%s\" at offset 0x%lx", update_part->label, (unsigned long)update_part->address);
